...
張曼娟大學堂是一個以張曼娟作家品牌為核心的線上文學學堂，不以一般課程商城為主要定位，而是結合會員制、付費課程、直播教室、回放、講義、文本共讀與站內 Q&A 的文學學習平台。
...
- `/`：首頁
  ...
  - 課程入口
  - 會員方案入口
  - 所有訪客可在「原版」與「窗景版」之間切換
  - 切換偏好保存於 `home-design` Cookie，網址同步支援 `?design=original` 與 `?design=window`
  - 兩版共用同一次精選課程查詢、功能路由與資料查詢失敗時的容錯入口
...
- Email 排程提醒
- 文章 / 短講 / 音頻內容庫
- 2026-06-30：前台頁首移除「管理員登入」連結，僅保留頁尾管理員入口，降低後台入口在公開頁面的可見度；管理員顯示名稱統一為「我輩學堂管理員」。
- 2026-07-01：後台頁面偵測 URL 中的 `saved` / `updated` 參數後，自動捲回頁面頂部，方便儲存後立即看到成功或更新提示。
- 2026-07-01：首頁與認識作家頁的張曼娟照片改用壓縮後 WebP 圖檔，原 6.4MB JPG 改為約 49KB WebP，以改善首頁圖片載入速度。
...
## 22. 2026-08-22 全站品牌名稱更新

- 對外網站名稱由「我輩學堂」統一更新為「張曼娟大學堂」。
- 同步更新前台頁首、頁尾、SEO 標題、會員與課程文案、電子票券、管理後台、管理員通知及會員 Email 範本。
- 圓形文字標記由「我」調整為「張」；既有管理員資料若仍為舊預設名稱，僅在顯示時轉換為新品牌名稱，不修改個人自訂名稱。
- 保留套件名稱、資料庫識別碼、GitHub／Vercel 專案 slug、網址與歷史修改紀錄，避免因品牌文字變更破壞技術相容性或追溯性。
