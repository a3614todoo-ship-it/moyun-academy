...
function membershipPeriod(application: TemplateApplication) {
  ...
  return {
    startedAt,
    endedAt,
    text: `${formatDate(startedAt)}－${formatDate(endedAt)}`,
  };
}
...
匯款人姓名：${report?.payerName || "未取得"}`,
  ...
  if (type === EmailType.APPLICATION_APPROVED) {
    ...
    const period = membershipPeriod(application);
    return {
      subject,
      text: `${application.name} 您好：
...
會員方案：${application.plan.name}
會員期間：${period.text}
會員效期：${application.plan.durationDays} 天
${groupUrl ? `Facebook 私密社團：${groupUrl}` : "Facebook 私密社團連結將由工作人員另行提供。"}`,
      html: emailShell(
        ...
        <p style="margin:18px 0 0;color:#69726d;font-size:14px;line-height:1.8;">會員期間自審核通過日起算，期間內可依學堂安排觀看完整課程與參與會員社群。</p>
